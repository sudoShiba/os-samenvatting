...
Dit bestand bevat de implementaties voor `kernel/trap.c`. Hieronder vind je de uitleg en de broncode van **elke functie** in dit bestand.
...
## `usertrap()`
>  handle an interrupt, exception, or system call from user space. called from, and returns to, trampoline.S return value is user satp for trampoline.S to switch to. 
